  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-stone-900 dark:text-white">Cash Flow Forecast</h1>
          <p className="text-stone-500 dark:text-stone-400 text-sm">Projected balances based on recurring bills and income</p>
        </div>
        <div className="flex gap-2">
          {[30, 60, 90].map(d => (
            <button key={d} onClick={() => setDays(d)}
              className={`px-3 py-1.5 text-sm rounded-lg ${days === d ? 'bg-teal-600 text-white' : 'bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 hover:bg-stone-200 dark:hover:bg-stone-700'}`}>
              {d} days
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="card p-4">
          <p className="text-sm text-stone-500 dark:text-stone-400">Current Balance</p>
          <p className="text-xl font-bold text-stone-900 dark:text-white">{formatCurrency(firstDay?.projected_balance || 0)}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-stone-500 dark:text-stone-400">Projected ({days}d)</p>
          <p className="text-xl font-bold text-stone-900 dark:text-white">{formatCurrency(lastDay?.projected_balance || 0)}</p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-stone-500 dark:text-stone-400">Net Change</p>
          <p className={`text-xl font-bold ${balanceChange >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {balanceChange >= 0 ? '+' : ''}{formatCurrency(balanceChange)}
          </p>
        </div>
        <div className="card p-4">
          <p className="text-sm text-stone-500 dark:text-stone-400">Min Balance</p>
          <p className={`text-xl font-bold ${hasNegative ? 'text-red-600' : 'text-stone-900 dark:text-white'}`}>
      ...
      <div className="card p-6">
        <h3 className="font-semibold text-stone-900 dark:text-white mb-4">Balance Projection</h3>
        <div className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={chartData}>
              ...
              <Area type="monotone" dataKey="balance" stroke="#14b8a6" fill="#14b8a6" fillOpacity={0.1} name="Projected" />
      ...
      {/* Scenario Builder */}
      <div className="card p-6">
        <h3 className="font-semibold text-stone-900 dark:text-white mb-4">What-If Scenario</h3>
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-[160px]">
            <label className="block text-sm text-stone-600 dark:text-stone-400 mb-1">Expense Name</label>
            <input type="text" value={scenarioName} onChange={e => setScenarioName(e.target.value)}
              placeholder="e.g., New Car Payment" className="w-full px-3 py-2 border rounded-lg dark:bg-stone-800 dark:border-stone-600 dark:text-white" />
          </div>
          <div className="w-32">
            <label className="block text-sm text-stone-600 dark:text-stone-400 mb-1">Amount</label>
            <input type="number" value={scenarioAmount} onChange={e => setScenarioAmount(e.target.value)}
              placeholder="500" className="w-full px-3 py-2 border rounded-lg dark:bg-stone-800 dark:border-stone-600 dark:text-white" />
          </div>
          <div className="w-36">
            <label className="block text-sm text-stone-600 dark:text-stone-400 mb-1">Frequency</label>
          ...
          <button onClick={() => setShowScenario(!showScenario)}
            className={`px-4 py-2 rounded-lg ${showScenario ? 'bg-amber-500 text-white' : 'bg-teal-600 text-white hover:bg-teal-700'}`}>
            ...
            {upcomingEvents.map((e, i) => (
              <div key={i} className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-stone-900 dark:text-white">{e.name}</span>
                  <span className="text-xs text-stone-400">{format(new Date(e.date), 'MMM d')}</span>
                </div>
