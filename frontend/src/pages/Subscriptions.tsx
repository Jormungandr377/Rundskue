    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-surface-500 dark:text-surface-400 text-sm">Track recurring charges and find unused subscriptions</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => detectMutation.mutate()} disabled={detectMutation.isPending}
            className="flex items-center gap-2 px-4 py-2 border border-surface-300 dark:border-surface-600 text-surface-700 dark:text-surface-300 rounded-xl hover:bg-surface-50 dark:hover:bg-surface-800">
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="card p-4">
            <p className="text-sm text-surface-500 dark:text-surface-400">Monthly Cost</p>
            <p className="text-xl font-bold text-surface-900 dark:text-white">{formatCurrency(summary.total_monthly_cost)}</p>
          </div>
          <div className="card p-4">
            <p className="text-sm text-surface-500 dark:text-surface-400">Annual Cost</p>
            <p className="text-xl font-bold text-surface-900 dark:text-white">{formatCurrency(summary.total_annual_cost)}</p>
          </div>
          <div className="card p-4">
          ...
          <div className="card p-4">
            <p className="text-sm text-surface-500 dark:text-surface-400">Possibly Unused</p>
      ...
      {showCreate && (
        <div className="card p-4 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-surface-900 dark:text-white">Add Subscription</h2>
          ...
          <form onSubmit={handleSubmit} className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[180px]">
              ...
              <input type="text" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" placeholder="Netflix" />
            </div>
            <div className="w-32">
              ...
              <input type="number" step="0.01" required value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
                className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" placeholder="15.99" />
            </div>
            <div className="w-36">
            ...
            <button type="submit" className="px-4 py-2 bg-primary-600 text-white rounded-xl hover:bg-primary-700">Add</button>
          </form>
        </div>
      )}
      ...
      {activeSubs.length === 0 && inactiveSubs.length === 0 ? (
        <div className="card p-12 text-center">
          <CreditCard className="w-12 h-12 text-surface-300 dark:text-surface-600 mx-auto mb-4" />
          <p className="text-surface-500 dark:text-surface-400 mb-2">No subscriptions tracked yet</p>
          <p className="text-sm text-surface-400">Click "Auto-Detect" to scan your transactions for recurring charges</p>
        </div>
      ) : (
        <div className="space-y-6">
          {activeSubs.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-surface-900 dark:text-white mb-3">Active ({activeSubs.length})</h2>
          ...
          {inactiveSubs.length > 0 && (
            <div>
              <h2 className="text-lg font-semibold text-surface-500 dark:text-surface-400 mb-3">Inactive ({inactiveSubs.length})</h2>
              <div className="card divide-y divide-surface-200 dark:divide-surface-700 opacity-60">
  ...
  return (
    <div className="flex items-center justify-between p-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="font-medium text-surface-900 dark:text-white">{sub.name}</span>
      ...
      <div className="flex items-center gap-4">
        <span className="font-semibold text-surface-900 dark:text-white">{formatCurrency(sub.amount)}</span>
        <button onClick={() => onToggle(!sub.is_active)}
          className={`text-xs px-2 py-1 rounded ${sub.is_active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-surface-100 text-surface-500 dark:bg-surface-800 dark:text-surface-400'}`}>
          {sub.is_active ? 'Active' : 'Paused'}
        </button>
        <button onClick={onDelete} className="text-surface-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
      </div>
    </div>
  );
}
