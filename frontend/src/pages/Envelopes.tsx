  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 dark:text-white">Envelopes</h1>
          <p className="text-surface-500 dark:text-surface-400 text-sm">Zero-based budgeting — give every dollar a job</p>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: 'Envelopes', value: String(summary.envelope_count), color: 'text-surface-900 dark:text-white' },
          ].map((card) => (
      ...
      {showCreate && (
        <div className="card p-4 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-surface-900 dark:text-white">{editingId ? 'Edit Envelope' : 'New Envelope'}</h2>
            <button onClick={() => { setShowCreate(false); setEditingId(null); }}><X className="w-5 h-5 text-surface-400" /></button>
          </div>
          <form onSubmit={handleSubmit} className="flex flex-wrap gap-4 items-end">
            <div className="flex-1 min-w-[200px]">
              ...
              <input type="text" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" placeholder="e.g., Groceries" />
            </div>
            <div className="w-40">
              ...
              <div className="flex gap-1">
                {COLORS.map(c => (
                  <button key={c} type="button" onClick={() => setForm(f => ({ ...f, color: c }))}
                    className={`w-7 h-7 rounded-full border-2 ${form.color === c ? 'border-surface-900 dark:border-white' : 'border-transparent'}`}
      ...
      {envList.length === 0 ? (
        <div className="card p-12 text-center">
          <Wallet className="w-12 h-12 text-surface-300 dark:text-surface-600 mx-auto mb-4" />
          <p className="text-surface-500 dark:text-surface-400">No envelopes yet. Create one to start budgeting!</p>
        </div>
      ) : (
              ...
              <div key={env.id} className="card p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-surface-900 dark:text-white">{env.name}</h3>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => startEdit(env)} className="p-1 text-surface-400 hover:text-surface-600"><Pencil className="w-4 h-4" /></button>
                    <button onClick={() => deleteMutation.mutate(env.id)} className="p-1 text-surface-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={isOver ? 'text-red-600 font-medium' : 'text-surface-900 dark:text-white'}>{formatCurrency(env.spent_amount)}</span>
                  </div>
                  <div className="w-full bg-surface-200 dark:bg-surface-700 rounded-full h-2.5">
                    <div className={`h-2.5 rounded-full transition-all ${isOver ? 'bg-red-500' : 'bg-primary-500'}`}
                      style={{ width: `${pct}%` }} />
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-surface-500 dark:text-surface-400">of {formatCurrency(env.allocated_amount)}</span>
                    <span className={`font-medium ${env.remaining_amount < 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {formatCurrency(env.remaining_amount)} left
                    </span>
                  </div>
                  <p className="text-xs text-surface-400">{env.transaction_count} transactions</p>
                </div>
              </div>
            );
          })}
