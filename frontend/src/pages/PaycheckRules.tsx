  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-surface-900 dark:text-white">Paycheck Splitting</h1>
          <p className="text-surface-500 dark:text-surface-400 text-sm">Auto-allocate income to envelopes, goals, and categories</p>
        </div>
      ...
      {showForm && (
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-surface-900 dark:text-white">{editingId ? 'Edit Rule' : 'New Rule'}</h2>
            <button onClick={resetForm}><X className="w-5 h-5 text-surface-400" /></button>
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">Rule Name</label>
                <input type="text" required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                  className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" placeholder="e.g., Biweekly Paycheck" />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">Match Merchant</label>
                <input type="text" required value={form.match_merchant} onChange={e => setForm(f => ({ ...f, match_merchant: e.target.value }))}
                  className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" placeholder="e.g., DFAS" />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">Min Amount (optional)</label>
                <input type="number" step="0.01" value={form.match_amount_min} onChange={e => setForm(f => ({ ...f, match_amount_min: e.target.value }))}
                  className="w-full px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" placeholder="0.00" />
              </div>
              <div>
                <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-1">Max Amount (optional)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-surface-700 dark:text-surface-300 mb-2">Allocations</label>
                ...
                {form.allocations.map((alloc, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <select value={alloc.target_type} onChange={e => updateAllocation(i, 'target_type', e.target.value)}
                      className="px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white">
                    ...
                    <input type="number" placeholder="Target ID" value={alloc.target_id || ''} onChange={e => updateAllocation(i, 'target_id', Number(e.target.value))}
                      className="w-24 px-3 py-2 border rounded-lg dark:bg-surface-800 dark:border-surface-600 dark:text-white" />
                    ...
                    <div className="relative w-28">
                      ...
                      {alloc.amount_type === 'percentage' ? (
                        <Percent className="absolute right-3 top-2.5 w-4 h-4 text-surface-400" />
                      ) : (
                        <DollarSign className="absolute right-3 top-2.5 w-4 h-4 text-surface-400" />
                      )}
                    </div>
                    {form.allocations.length > 1 && (
                      <button type="button" onClick={() => removeAllocation(i)} className="p-2 text-surface-400 hover:text-red-500">
              ...
              <button type="button" onClick={addAllocation} className="mt-2 text-sm text-primary-600 hover:text-primary-700">+ Add Allocation</button>
            </div>

            <div className="flex justify-end gap-3">
              <button type="button" onClick={resetForm} className="px-4 py-2 text-surface-600 hover:bg-surface-100 rounded-lg">Cancel</button>
      ...
      {rules.length === 0 ? (
        <div className="card p-12 text-center">
          <DollarSign className="w-12 h-12 text-surface-300 dark:text-surface-600 mx-auto mb-4" />
          <p className="text-surface-500 dark:text-surface-400">No paycheck rules yet. Create one to auto-allocate income!</p>
        </div>
      ) : (
          ...
          {rules.map(rule => (
            <div key={rule.id} className="card p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-semibold text-surface-900 dark:text-white">{rule.name}</h3>
                ...
                <div className="flex gap-1">
                  <span className={`text-xs px-2 py-0.5 rounded-full ${rule.is_active ? 'bg-green-100 text-green-700' : 'bg-surface-100 text-surface-500'}`}>
                    {rule.is_active ? 'Active' : 'Inactive'}
                  </span>
                  <button onClick={() => startEdit(rule)} className="p-1 text-surface-400 hover:text-surface-600"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => deleteMutation.mutate(rule.id)} className="p-1 text-surface-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              {rule.allocations.length > 0 && (
                <div className="border-t border-surface-100 dark:border-surface-700 pt-2 mt-2">
                  <p className="text-xs text-surface-400 mb-1">Allocations:</p>
                  <div className="flex flex-wrap gap-2">
                    {rule.allocations.map((a, i) => (
                      <span key={i} className="text-xs bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300 px-2 py-1 rounded">
